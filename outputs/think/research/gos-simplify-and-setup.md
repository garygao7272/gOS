...
### Confirmed Critical Gaps (after direct verification of install.sh)

| # | Gap | Evidence | Fix |
|---|-----|----------|-----|
| **C1** | `~/.claude/invariants.md` has **no source** in gOS repo; install.sh never copies | both agents; verified: `find gOS -name invariants.md` = 0, `grep invariants install.sh` = 0 | Add `gOS/invariants.md` + install.sh copy step |
| **C2** | `claws/` never installed (no block in install.sh) | parity; verified: `grep claws install.sh` = 0; `gOS/claws/` has 3 claws | Add `claws/` copy block |

### False Positives (caught on direct verification of install.sh)

- ~~"install.sh reads hooks from wrong path"~~ — line 248 reads `$GOS_DIR/.claude/hooks/*` which resolves correctly; 30 hooks copy fine. Auditor error.
- ~~"TypeScript MCPs never npm-installed"~~ — hyperliquid-mcp DOES `npm install` (line 161). spec-rag and sources are Python (venvs, not npm). Correct but narrower than stated.
