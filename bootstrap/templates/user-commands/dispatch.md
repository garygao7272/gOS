...
| `<plan-file> --auto` | Start a dispatch job without interactive approval (mobile-safe) |
...
Wait for explicit approval. **If `--auto` flag is set:** skip approval and proceed immediately. Write "Auto-approved dispatch job" to `sessions/mobile-output.md`.
