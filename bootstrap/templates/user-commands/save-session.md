...
After saving the session state (Step 5), ask the user these 3 questions:

1. **"What worked well this session?"** (approach, tool, pattern — anything worth repeating)
2. **"What didn't work or took too long?"** (anti-pattern, wrong tool, dead end — anything to avoid)
3. **"What should I do differently next time?"** (one actionable change for future sessions)

Save the answers to a memory file at the project memory path:
`~/.claude/projects/<project-slug>/memory/feedback_session_YYYY-MM-DD.md`
...
Then update the project's `MEMORY.md` to add a line under the `## Feedback` section linking to the new file:
```markdown
- [Session Feedback YYYY-MM-DD](feedback_session_YYYY-MM-DD.md) — [one-line summary of key learning]
```

If no `## Feedback` section exists in `MEMORY.md`, create one.
